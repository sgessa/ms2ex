defmodule Ms2ex.Formulas.ItemWeaponAttack do
  ...
  @slot_coefficients %{30 => 1.0, 31 => 1.03, 32 => 0.95, 33 => 0.92, 34 => 0.95, 50 => 1.1, 51 => 1.03, 52 => 1.2, 53 => 1.05, 54 => 1.0, 55 => 1.13, 56 => 0.96}
  @speed_coefficients %{30 => 0.95, 31 => 1.05, 32 => 1.0, 33 => 1.1, 34 => 1.0, 50 => 0.95, 51 => 1.05, 52 => 1.0, 53 => 0.9, 54 => 1.0, 55 => 1.05, 56 => 0.95}
  @deviations %{1 => %{30 => 0.05, 31 => 0.15, 32 => 0.02, 33 => 0.05, 34 => 0.15, 50 => 0.1, 51 => 0.05, 52 => 0.05, 53 => 0.1, 54 => 0.0, 55 => 0.05, 56 => 0.1}, 2 => %{30 => 0.1, 31 => 0.3, 32 => 0.04, 33 => 0.1, 34 => 0.3, 50 => 0.2, 51 => 0.1, 52 => 0.1, 53 => 0.2, 54 => 0.0, 55 => 0.1, 56 => 0.2}}
  @grade_coefficients %{1 => %{1 => 0.9, 2 => 0.98, 3 => 1.06, 4 => 1.14, 5 => 1.21, 6 => 1.4}, 2 => %{1 => 0.9, 2 => 0.98, 3 => 1.06, 4 => 1.14, 5 => 1.21, 6 => 1.4}}
  @static_coefficients %{1 => 0.1, 2 => 0.12, 3 => 0.14, 4 => 0.16, 5 => 0.18, 6 => 0.2}

  def constant_value(:min, base, deviation, type, _job, factor, grade, level), do: constant(base, deviation, type, trunc(factor), grade, level, :min)
  def constant_value(:max, base, deviation, type, _job, factor, grade, level), do: constant(base, deviation, type, trunc(factor), grade, level, :max)
  ...
  def static_value(:max, base, deviation, type, _job, factor, grade, level) do
    ...
    max_value = max(slot * Map.fetch!(@static_coefficients, grade) * (1 + deviation(type, 1)), 2) + add
    min_value = max(max_value * 0.78, 1)
    if base == 0, do: {min_value, max_value}, else: {rounded(max_value * base / 100), rounded(max_value * base / 100)}
  end
  ...
  defp constant(base, dev, type, factor, grade, level, bound) do
    ...
    if level >= 70 and base > 0, do: rounded(base_value * base / 100) + rounded(add * sign * base / 100), else: base_value + add |> max(0)
  end

  defp constant_add(_base, _dev, _type, _factor, grade, level) when grade < 4 or level <= 49, do: 0
  defp constant_add(_base, dev, type, factor, grade, level) do
    ...
    grade_value = ((rounded(slot * grade_coefficient(dev, grade) * (1 - deviation(type, dev))) + rounded(slot * grade_coefficient(dev, grade) * (1 + deviation(type, dev)))) / 2) * Map.get(table, grade, 0)
    epic = ((rounded(slot * grade_coefficient(dev, 4) * (1 - deviation(type, dev))) + rounded(slot * grade_coefficient(dev, 4) * (1 + deviation(type, dev)))) / 2) * Map.get(table, 4, 0)
  ...
  defp static_add(_base, _dev, type, factor, grade, level) do
    effective_type = if type == 54, do: 52, else: type
    slot = rounded(level_base(factor, level) * coefficient(effective_type) / speed(effective_type))
  ...
  defp level_base(factor, level) do
    Enum.reduce(2..factor, 5, fn index, local -> local + if(index > 49 and level >= 60, do: local * @upgrade_factor, else: max(index / 30 * 20 - 0.8, 0)) end)
  end
