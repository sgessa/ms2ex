defmodule Ms2ex.Managers.Character do
  ...
  alias Ms2ex.{Context, Packets, PartyServer, Schema, Types}
  ...
  def cast_skill(%Schema.Character{} = character, %Types.SkillCast{} = skill_cast) do
  ...
  def handle_call({:cast_skill, skill_cast}, _from, %{stats: stats} = character) do
    ...
    if stats.spirit_cur >= spirit_cost and stats.stamina_cur >= stamina_cost do
      ...
      if Types.SkillCast.owner_buff?(skill_cast) or Types.SkillCast.entity_buff?(skill_cast) or
           Types.SkillCast.shield_buff?(skill_cast) or
           Types.SkillCast.owner_debuff?(skill_cast) do
