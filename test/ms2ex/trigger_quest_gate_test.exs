defmodule Ms2ex.TriggerQuestGateTest do
  ...
  test "wanted 1 matches a started quest with conditions unmet" do
    assert Conditions.quest_state_matches?(started_quest(), 1)
  end

  test "wanted 2 does not match before the carry completes" do
    refute Conditions.quest_state_matches?(started_quest(), 2)
  end

  test "wanted 2 matches once the item_move condition is met" do
    quest =
      Ms2ex.Managers.Quest.Conditions.update(
        started_quest(),
        :item_move,
        1,
        "",
    ...
    refute Conditions.quest_state_matches?(quest, 1)
    assert Conditions.quest_state_matches?(quest, 2)
  end
  ...
  test "wanted 3 matches only a completed quest" do
    refute Conditions.quest_state_matches?(started_quest(), 3)
    assert Conditions.quest_state_matches?(%{started_quest() | state: :completed}, 3)
  end

  test "wanted 2 no longer matches a completed quest" do
    refute Conditions.quest_state_matches?(%{started_quest() | state: :completed}, 2)
  end

  test "a missing quest never matches" do
    refute Conditions.quest_state_matches?(nil, 1)
    refute Conditions.quest_state_matches?(nil, 2)
    refute Conditions.quest_state_matches?(nil, 3)
  end
end
